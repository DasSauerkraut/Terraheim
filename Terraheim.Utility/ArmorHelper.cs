...
internal class ArmorHelper
{
	...
	public static void ModArmorSet(string setName, ref ItemDrop.ItemData helmet, ref ItemDrop.ItemData chest, ref ItemDrop.ItemData legs, JToken values, bool isNewSet, int i)
	{
		...
		helmet.m_shared.m_armor += armorSet.HelmetArmor;
	...
	public static void ModArmorPiece(string setName, string location, ref ItemDrop.ItemData piece, JToken values, bool isNewSet, int i)
	{
		...
		if (location == "head")
		{
			piece.m_shared.m_armor += armorSet.HelmetArmor;
		}
	...
	{
		JToken jToken = balance[setName];
		Data.ArmorSet armorSet = Data.ArmorSets[setName];
		for (int i = (int)jToken["upgrades"]!["startingTier"]; i <= 6; i++)
		{
			...
			{
			case "head":
				text = armorSet.HelmetID;
				arg = armorSet.HelmetName;
				break;
			case "chest":
				text = armorSet.ChestID;
				arg = armorSet.ChestName;
				break;
			case "legs":
				text = armorSet.LegsID;
				arg = armorSet.LegsName;
				break;
			}
			...
			ModArmorPiece(setName, location, ref customItem.ItemDrop.m_itemData, jToken, isNewSet: true, i);
			Recipe recipe = ScriptableObject.CreateInstance<Recipe>();
			recipe.name = $"Recipe_{text}T{i}";
			List<Piece.Requirement> list = new List<Piece.Requirement>();
			int num = 0;
			if (i == (int)jToken["upgrades"]!["startingTier"])
			{
